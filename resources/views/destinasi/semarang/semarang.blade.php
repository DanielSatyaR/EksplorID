...
<body>
    ...
    <div class="bg-gray-50 min-h-screen">
        ...
        <div class="relative w-full h-72">
            <img src="img/semarang1.jpg" alt="Banner Image" class="w-full h-full object-cover">
            <div class="absolute inset-0 bg-black bg-opacity-40 flex justify-center items-center">
                <h1 class="text-4xl text-white font-bold">Semarang</h1>
            </div>
        </div>
        ...
        <div class="container mx-auto p-6 flex space-x-6">
            ...
            <!-- Product Section -->
            @foreach ($destinasiSemarang as $destinasi)
            <div class="w-3/2 grid grid-cols-3 gap-3 flex flex-col h-72">
                <!-- Product Card -->
                <div class="bg-white p-4 rounded-lg shadow-md">
                    <a href="{{ route('semarang.show', $destinasi->slug) }}">
                        @if($destinasi->images->isNotEmpty())
                        <img src="{{ asset('storage/' . $destinasi->images->first()->image) }}" alt="Destinasi Image" class="w-full h-48 object-cover rounded-lg mb-4">
                        @endif
                        <div class="text-lg font-semibold">{{ $destinasi->title }}</div>
                        <div class="text-gray-500">{{ $destinasi->category->name ?? 'Uncategorized' }}</div>
                        <div class="mt-2 text-xl font-semibold">Rp. {{ $destinasi->price }}</div>
                    </a>
                </div>
            </div>
            @endforeach
